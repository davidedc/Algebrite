...
(function() {
  ...
  test_dependencies = function() {
    ...
    if (testResult.code === "" && testResult.latexResult === "$$3y+6$$" && testResult.result === "$$3y+6$$" && testResult.dependencyInfo.affectedBy.length === 1 && testResult.dependencyInfo.affectedBy[0] === "PATTERN_DEPENDENCY" && testResult.dependencyInfo.affectsVariables.length === 0) {
      console.log("ok dependency test");
    } else {
  ...
  findDependenciesInScript = function(stringToBeParsed, dontGenerateCode) {
    var ae, af, ag, ah, aj, al, allReturnedLatexStrings, allReturnedPlainStrings, am, ao, bodyForReadableSummaryOfGeneratedCode, cacheKey, cacheMissPenalty, currentStateHash, cyclesDescriptions, deQuotedDep, dependencyInfo, eachDependency, error, error1, error2, error3, frozen, generatedBody, generatedCode, i, indexOfEachReplacement, indexOfPartRemainingToBeParsed, inited, key, len, len1, len2, len3, len4, len5, len6, n, newUserSymbol, originalUserSymbol, parameters, possiblyCached, readableSummaryOfGeneratedCode, recursedDependencies, ref2, replacementsFrom, replacementsTo, scriptEvaluation, stringToBeRun, testableString, timeStartFromAlgebra, toBeFrozen, toBePrinted, totalTime, userVariablesMentioned, value, variablesWithCycles;
    if (DEBUG) {
    ...
    if (errorMessage === "" && !dontGenerateCode) {
      ...
      if (errorMessage === "") {
        for (key in symbolsDependencies) {
          ...
          for (indexOfEachReplacement = am = 0, ref2 = replacementsFrom.length; 0 <= ref2 ? am < ref2 : am > ref2; indexOfEachReplacement = 0 <= ref2 ? ++am : --am) {
          ...
          if (errorMessage === "") {
            ...
            if (variablesWithCycles.indexOf(key) !== -1) {
              ...
              if (userVariablesMentioned.length !== 0) {
                parameters = "(";
                for (ao = 0, len6 = userVariablesMentioned.length; ao < len6; ao++) {
                  i = userVariablesMentioned[ao];
                  if (i.printname !== key) {
