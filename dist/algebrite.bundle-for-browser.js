...
(function() {
  ...
  test_dependencies = function() {
    ...
    if (testResult[0] === "All local dependencies:  variable x depends on: sqrt, ; . All dependencies recursively:  variable x depends on: sqrt, ; " && testResult[1] === "" && testResult[2] === "x = function (sqrt) { return ( -1 / (Math.pow(2, (1/2))) ); }") {
      console.log("ok dependency test");
    } else {
      console.log("fail dependency test. expected: " + testResult);
    }
    ...
    if (testResult[0] === "All local dependencies:  variable x depends on: a, ; . All dependencies recursively:  variable x depends on: a, ; " && testResult[1] === "" && testResult[2] === "x = function (a) { return ( 1 / (5*Math.pow(2, (1/2))) ); }") {
      console.log("ok dependency test");
    } else {
      console.log("fail dependency test. expected: " + testResult);
    }
    ...
    if (testResult[0] === "All local dependencies:  variable g depends on: 'x, f, x, ;  variable f depends on: 'x, g, x, ; . All dependencies recursively:  variable g depends on: 'x, x, ;  g --> f -->  ... then g again,  variable f depends on: 'x, x, ;  f --> g -->  ... then f again, " && testResult[1] === "" && testResult[2] === "// g is part of a cyclic dependency, no code generated.\n// f is part of a cyclic dependency, no code generated.") {
    ...
    if (testResult[0] === "All local dependencies:  variable f depends on: a, b, c, ; . All dependencies recursively:  variable f depends on: a, b, c, ; " && testResult[1] === "" && testResult[2] === "f = function (a, b, c) { return ( [-(b + Math.pow((-4*a*c + Math.pow(b, 2)), (1/2))) / (2*a),(-b + Math.pow((-4*a*c + Math.pow(b, 2)), (1/2))) / (2*a)] ); }") {
  ...
  findDependenciesInScript = function(stringToBeParsed) {
    var ae, af, ag, ah, aj, allReturnedStrings, bodyForReadableSummaryOfGeneratedCode, cyclesDescriptions, error, error1, generatedBody, generatedCode, i, indexOfPartRemainingToBeParsed, inited, key, len, len1, len2, len3, len4, n, parameters, readableSummaryOfGeneratedCode, recursedDependencies, scriptEvaluation, testableString, toBePrinted, value, variablesWithCycles;
    inited = true;
    symbolsDependencies = {};
  ...
  computeResultsAndJavaScriptFromAlgebra = function(codeFromAlgebraBlock) {
    var code, readableSummaryOfCode, ref2, result, testableStringIsIgnoredHere;
    clear_symbols();
    defn();
    ref2 = findDependenciesInScript(codeFromAlgebraBlock), testableStringIsIgnoredHere = ref2[0], result = ref2[1], code = ref2[2], readableSummaryOfCode = ref2[3];
