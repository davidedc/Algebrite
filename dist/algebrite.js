...
(function() {
  ...
  cached_computeResultsAndJavaScriptFromAlgebra = null;
  ...
  findDependenciesInScript = function(stringToBeParsed, dontGenerateCode) {
    var ae, af, ag, ah, aj, al, allReturnedLatexStrings, allReturnedPlainStrings, am, ao, bodyForReadableSummaryOfGeneratedCode, cyclesDescriptions, deQuotedDep, dependencyInfo, eachDependency, error, error1, error2, error3, generatedBody, generatedCode, i, indexOfEachReplacement, indexOfPartRemainingToBeParsed, inited, key, len, len1, len2, len3, len4, len5, len6, n, newUserSymbol, originalUserSymbol, parameters, readableSummaryOfGeneratedCode, recursedDependencies, ref2, replacementsFrom, replacementsTo, scriptEvaluation, testableString, toBePrinted, userVariablesMentioned, value, variablesWithCycles;
    if (DEBUG) {
  ...
  computeResultsAndJavaScriptFromAlgebra = function(codeFromAlgebraBlock) {
    var ae, anyErrors, cacheKey, code, currentStateHash, dependencyInfo, frozen, i, keepState, latexResult, len, originalcodeFromAlgebraBlock, possiblyCached, readableSummaryOfCode, ref2, result, stringToBeRun, testableStringIsIgnoredHere, timeStartFromAlgebra, toBeFrozen, totalTime, userSimplificationsInProgramForm;
    ...
    if (ENABLE_CACHING && stringToBeRun !== "clearall") {
      ...
      cacheKey = currentStateHash + " stringToBeRun: " + stringToBeRun;
      if (CACHE_DEBUGS) {
        console.log("cached_computeResultsAndJavaScriptFromAlgebra key: " + cacheKey);
      }
      possiblyCached = cached_computeResultsAndJavaScriptFromAlgebra.get(cacheKey);
      if (possiblyCached != null) {
        if (CACHE_HITSMISS_DEBUGS) {
          console.log("cache_computeResultsAndJavaScriptFromAlgebra hit on " + stringToBeRun);
        }
        unfreeze(possiblyCached);
        if (TIMING_DEBUGS) {
        ...
        if (CACHE_HITSMISS_DEBUGS) {
          console.log("cached_computeResultsAndJavaScriptFromAlgebra miss on: " + stringToBeRun);
        }
        if (TIMING_DEBUGS) {
    ...
    if (ENABLE_CACHING && stringToBeRun !== "clearall" && !anyErrors) {
      frozen = freeze();
      ...
      if (CACHE_DEBUGS) {
        console.log("setting cached_computeResultsAndJavaScriptFromAlgebra on key: " + cacheKey);
      }
  ...
  resetCache = function() {
    ...
    if (cached_computeResultsAndJavaScriptFromAlgebra != null) {
      cached_computeResultsAndJavaScriptFromAlgebra.reset();
      if (DEBUG) {
        return console.log("resetting cached_computeResultsAndJavaScriptFromAlgebra");
      }
    }
  };
