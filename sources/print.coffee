...
#   a = 1
#   b = 2
# and from this point on getting the actual
...
# evaluating it.
# If it's passed any other type of expression
# then it just prints it as is, again without
# evaluating it.
#
...
#
# Since the working workaround of the two nested quotes

Eval_printunevalled_list = ->
	p1 = car(cdr(p1))
	if !iscons(p1) and cadr(p1).k == SYM
		console.log "getting the variab"
		p1 = get_binding(p1)
	print1 p1
	push(symbol(NIL))	

	# if one wanted to print a normal
	# evalled version one would do
	# instead:
	#push car(cdr(p1))
	#Eval()
	#p1 = pop()
