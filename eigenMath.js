...
s = ["condense(a/(a+b)+b/(a+b))", "1", "psi(n) = exp(-r/n) laguerre(2r/n,n-1,1)", "", "psi(3)", "3*exp(-1/3*r)-2*r*exp(-1/3*r)+2/9*r^2*exp(-1/3*r)", "condense(last)", "exp(-1/3*r)*(3-2*r+2/9*r^2)", "psi()=psi", "", "condense(-3 exp(-1/3 r + i phi) cos(theta) - 6 exp(-1/3 r + i phi) cos(theta) sin(theta)^2 + 12 exp(-1/3 r + i phi) cos(theta)^3)", "3*exp(-1/3*r+i*phi)*(-1+4*cos(theta)^2-2*sin(theta)^2)*cos(theta)"];
...
set_component = function(n) {
  var ad, ae, af, ag, ah, ai, i, k, m, ndim, ref1, ref2, ref3, ref4, ref5, ref6, ref7, t;
  i = 0;
  k = 0;
  m = 0;
  ndim = 0;
  ...
  for (i = ai = 0, ref7 = p2.tensor.nelem; 0 <= ref7 ? ai < ref7 : ai > ref7; i = 0 <= ref7 ? ++ai : --ai) {
...
inner_f = function() {
  var a, ad, ae, af, ag, ah, ai, aj, ak, b, bk, c, i, j, k, n, ndim, ref1, ref2, ref3, ref4, ref5, ref6, ref7;
  i = 0;
  ...
  if (ndim === 0) {
    ...
    p3.tensor.ndim = ndim;
    j = 0;
    for (i = ai = 0, ref6 = p1.tensor.ndim - 1; 0 <= ref6 ? ai < ref6 : ai > ref6; i = 0 <= ref6 ? ++ai : --ai) {
      p3.tensor.dim[i] = p1.tensor.dim[i];
      j = i;
    }
    for (i = aj = 0, ref7 = p2.tensor.ndim - 1; 0 <= ref7 ? aj < ref7 : aj > ref7; i = 0 <= ref7 ? ++aj : --aj) {
