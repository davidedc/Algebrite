...
var ABS, ADD, ADJ, AND, ARCCOS, ARCCOSH, ARCSIN, ARCSINH, ARCTAN, ARCTANH, ARG, ATOMIZE, AUTOEXPAND, BAKE, BESSELJ, BESSELY, BINDING, BINOMIAL, BUF, C1, C2, C3, C4, C5, C6, CEILING, CHECK, CHOOSE, CIRCEXP, CLEAR, CLOCK, COEFF, COFACTOR, CONDENSE, CONJ, CONS, CONTRACT, COS, COSH, Condense, DEBUG, DECOMP, DEFINT, DEGREE, DENOMINATOR, DERIVATIVE, DET, DIM, DIRAC, DISPLAY, DIVISORS, DO, DOT, DOUBLE, DRAW, DRAWX, DSOLVE, E, EIGEN, EIGENVAL, EIGENVEC, ERF, ERFC, EVAL, EXP, EXPAND, EXPCOS, EXPSIN, Eval, Eval_Eval, Eval_abs, Eval_add, Eval_adj, Eval_and, Eval_arcsin, Eval_arctan, Eval_arg, Eval_besselj, Eval_bessely, Eval_binding, Eval_ceiling, Eval_check, Eval_choose, Eval_circexp, Eval_clear, Eval_clock, Eval_coeff, Eval_cofactor, Eval_condense, Eval_conj, Eval_cons, Eval_contract, Eval_cos, Eval_cosh, Eval_decomp, Eval_defint, Eval_denominator, Eval_derivative, Eval_det, Eval_dim, Eval_divisors, Eval_do, Eval_dsolve, Eval_exp, Eval_expcos, Eval_expsin, Eval_factor, Eval_factorial, Eval_factorpoly, Eval_float, Eval_gcd, Eval_hermite, Eval_hilbert, Eval_imag, Eval_index, Eval_inner, Eval_integral, Eval_inv, Eval_invg, Eval_isinteger, Eval_laguerre, Eval_lcm, Eval_log, Eval_mag, Eval_multiply, Eval_noexpand, Eval_not, Eval_number, Eval_numerator, Eval_operator, Eval_or, Eval_outer, Eval_power, Eval_predicate, Eval_print, Eval_quote, Eval_rank, Eval_rationalize, Eval_real, Eval_rect, Eval_setq, Eval_simplify, Eval_sin, Eval_sinh, Eval_sqrt, Eval_stop, Eval_subst, Eval_sym, Eval_tan, Eval_tanh, Eval_tensor, Eval_test, Eval_testeq, Eval_testge, Eval_testgt, Eval_testle, Eval_testlt, Eval_unit, Eval_user_function, Evalpoly, FACTOR, FACTORIAL, FACTORPOLY, FILTER, FLOATF, FLOOR, FOR, Find, GAMMA, GCD, HERMITE, HILBERT, IMAG, INDEX, INNER, INTEGRAL, INV, INVG, ISINTEGER, ISPRIME, LAGUERRE, LAST, LCM, LEADING, LEGENDRE, LOG, M, MAG, MAXDIM, MAXPRIMETAB, MAX_PROGRAM_SIZE, MEQUAL, METAA, METAB, METAX, MLENGTH, MOD, MP_MAX_FREE, MP_MIN_SIZE, MSIGN, MULTIPLY, MZERO, N, NIL, NOT, NROOTS, NSYM, NUM, NUMBER, NUMERATOR, OPERATOR, OR, OUTER, PI, POLAR, POWER, PRIME, PRINT, PRODUCT, QUOTE, QUOTIENT, RANK, RATIONALIZE, REAL, ROOTS, SECRETX, SELFTEST, SETQ, SGN, SIMPLIFY, SIN, SINH, SPACE_BETWEEN_COLUMNS, SPACE_BETWEEN_ROWS, SQRT, STOP, STR, SUBST, SUM, SYM, SYMBOL_A, SYMBOL_B, SYMBOL_C, SYMBOL_D, SYMBOL_I, SYMBOL_J, SYMBOL_N, SYMBOL_R, SYMBOL_S, SYMBOL_T, SYMBOL_X, SYMBOL_Y, SYMBOL_Z, TAN, TANH, TAYLOR, TENSOR, TEST, TESTEQ, TESTGE, TESTGT, TESTLE, TESTLT, TOS, TRACE, TRANSPOSE, TTY, T_DOUBLE, T_EQ, T_FUNCTION, T_GTEQ, T_INTEGER, T_LTEQ, T_NEWLINE, T_STRING, T_SYMBOL, U, UNIT, USR_SYMBOLS, YMAX, YYE, YYRECT, ZERO, __cmp, __emit_char, __emit_str, __factor_add, __factorial, __is_negative, __is_radical_number, __lcm, __normalize_radical_factors, __rationalize_tensor, absval, absval_tensor, ac, add, add_all, add_numbers, add_terms, addf, adj, alloc_tensor, allocatedId, any_denominators, arcsin, arctan, arg, arglist, bake, bake_poly, bake_poly_term, besselj, bessely, bignum_factorial, bignum_float, bignum_power_number, bignum_scan_float, bignum_scan_integer, bignum_truncate, binding, buffer, build_tensor, caaddr, caadr, caar, cadaddr, cadadr, cadar, caddaddr, caddadr, caddar, caddddr, cadddr, caddr, cadr, car, cdaddr, cdadr, cdar, cddaddr, cddar, cdddaddr, cddddr, cdddr, cddr, cdr, ceiling, charTabIndex, chartab, check_arg, check_esc_flag, check_stack, choose, choose_check_args, circexp, clear, clear_symbols, clear_term, clockform, cmpGlyphs, cmp_args, cmp_expr, cmp_terms, cmp_terms_count, coeff, cofactor, collectResultLine, combine_factors, combine_gammas, combine_terms, compare_numbers, compare_rationals, compare_tensors, compatible, conjugate, cons, consCount, contract, convert_bignum_to_double, convert_rational_to_double, copy_tensor, cosine, cosine_of_angle, cosine_of_angle_sum, count, count_denominators, counter, d_scalar_scalar, d_scalar_scalar_1, d_scalar_tensor, d_tensor_scalar, d_tensor_tensor, dabs, darccos, darccosh, darcsin, darcsinh, darctan, darctanh, dbesselj0, dbesseljn, dbessely0, dbesselyn, dcos, dcosh, dd, decomp, decomp_product, decomp_sum, define_user_function, defn, defn_str, denominator, derf, derfc, derivative, derivative_of_integral, det, determinant, detg, dfunction, dhermite, display, display_flag, displaychar, divide, divide_numbers, divisors, divisors_onstack, dlog, doubleToReasonableString, dpow, dpower, dproduct, draw_flag, draw_stop_return, dsgn, dsin, dsinh, dsum, dtan, dtanh, dupl, echo_input, egcd, elelmIndex, elem, emit_denominator, emit_denominators, emit_expr, emit_factor, emit_factorial_function, emit_flat_tensor, emit_fraction, emit_function, emit_index_function, emit_multiply, emit_number, emit_numerators, emit_numerical_fraction, emit_power, emit_string, emit_subexpr, emit_symbol, emit_tensor, emit_tensor_inner, emit_term, emit_top_expr, emit_unsigned_expr, emit_x, endian, equal, equaln, equalq, error, errorMessage, esc_flag, expanding, expcos, exponential, expr_level, expsin, f1, f2, f3, f4, f5, f9, f_equals_a, factor, factor_a, factor_again, factor_b, factor_number, factor_small_number, factor_term, factorial, factorpoly, factpoly_expo, fill_buf, fixup_fraction, fixup_power, flag, fmt_index, fmt_level, fmt_x, frame, free_stack, gcd, gcd_expr, gcd_expr_expr, gcd_factor_term, gcd_main, gcd_numbers, gcd_term_factor, gcd_term_term, ge, gen, get_arglist, get_binding, get_factor, get_next_token, get_printname, get_size, get_token, getdisplaystr, glyph, gp, guess, imag, imaginaryunit, index_function, init, inited, inner, inner_f, input_str, integral, integral_of_form, integral_of_product, integral_of_sum, inverse, invert_number, is_denominator, is_factor, is_small_integer, is_square_matrix, isadd, isalnum, isalpha, iscomplexnumber, iscons, isdenominator, isdigit, isdouble, iseveninteger, isfactor, isfactorial, isfloating, isfraction, isimaginarynumber, isimaginaryunit, isinteger, isintegerfactor, iskeyword, isminusone, isminusoneoversqrttwo, isnegative, isnegativenumber, isnegativeterm, isnonnegativeinteger, isnpi, isnum, isoneover, isoneoversqrttwo, isplusone, ispoly, ispoly_expr, ispoly_factor, ispoly_term, isposint, ispower, isquarterturn, isrational, isspace, isstr, issymbol, issymbolic, istensor, iszero, laguerre, laguerre2, lcm, length, lessp, level, list, logarithm, logbuf, logout, lookupsTotal, lu_decomp, madd, mag, makePositive, makeSignSameAs, mask, mcmp, mcmpint, mdiv, mdivrem, meta_mode, mgcd, mint, mmod, mmul, move, mp_clr_bit, mp_denominator, mp_numerator, mp_set_bit, mpow, mprime, mroot, mshiftright, msub, mtotal, multinomial_sum, multiply, multiply_all, multiply_all_noexpand, multiply_denominators, multiply_denominators_factor, multiply_denominators_term, multiply_noexpand, multiply_numbers, n_factor_number, negate, negate_expand, negate_noexpand, negate_number, new_string, newline_flag, nil_symbols, normalize_angle, nterms, numerator, o, one, oneElement, out_buf, out_count, out_of_memory, outer, p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, parse_p1, parse_p2, partition, peek, peek2, polycoeff, polyform, pop, pop_double, pop_frame, pop_integer, power, power_str, power_sum, power_tensor, primetab, print1, print_a_over_b, print_char, print_denom, print_double, print_expr, print_factor, print_factorial_function, print_it, print_multiply_sign, print_number, print_str, print_subexpr, print_tensor, print_tensor_inner, print_term, printchar, printchar_nowrap, printline, program_buf, promote_tensor, push, push_cars, push_double, push_factor, push_frame, push_identity_matrix, push_integer, push_rational, push_symbol, push_terms, push_zero_matrix, qadd, qdiv, qmul, qpow, qpowf, quickfactor, quickpower, rational, rationalize, rationalize_coefficients, real, reciprocate, rect, ref, restore, rewrite_args, rewrite_args_tensor, run, run_test, s, save, scalar_times_tensor, scan, scan_expression, scan_factor, scan_function_call, scan_meta, scan_power, scan_relation, scan_stmt, scan_str, scan_string, scan_subexpr, scan_symbol, scan_term, scanned, selftest, setM, setSignTo, set_binding, set_binding_and_arglist, set_component, setq_indexed, sfac_product, sfac_product_f, sign, sign_of_term, simplify, simplify_main, simplify_polar, simplify_tensor, simplify_trig, simplifyfactorials, sine, sine_of_angle, sine_of_angle_sum, sort_stack, square, ssqrt, stack, stackAddsCount, std_symbol, stop, strcmp, stringToBePrinted, subf, subst, subtract, subtract_numbers, swap, symbol, symnum, symtab, tangent, tensor, tensor_plus_tensor, tensor_times_scalar, test_abs, test_adj, test_arcsin, test_arg, test_bake, test_besselj, test_bessely, test_ceiling, test_choose, test_circexp, test_clock, test_cofactor, test_condense, test_contract, test_cosh, test_defint, test_expcos, test_expsin, test_factor_number, test_flag, test_float, test_laguerre, test_low_level, test_madd, test_maddf, test_mdiv, test_mdivf, test_mgcd, test_mmod, test_mmodf, test_mmul, test_mmulf, test_mpow, test_mprime, test_mroot, test_msub, test_msubf, test_multiply, test_outer, test_power, test_quickfactor, test_scan, test_sinh, test_tan, test_tanh, test_tensor, test_test, text_metric, token, token_buf, token_str, top_level_eval, tos, transform, trigmode, try_kth_prime, ucmp, unique, unique_f, update_token_buf, usr_symbol, verbosing, will_be_displayed_as_fraction, ycosh, yindex, ysinh, yyarg, yybesselj, yybessely, yyceiling, yycondense, yycontract, yycosh, yydetg, yydivpoly, yyexpand, yyfactorpoly, yyfloat, yylcm, yylog, yymag, yymultiply, yyouter, yypower, yyrationalize, yysinh, yytangent, zero;
...
s = ["denominator(2/3)", "3", "denominator(x)", "1", "denominator(1/x)", "x", "denominator(a+b)", "1", "denominator(1/a+1/b)", "a*b", "denominator(1/(x-1)/(x-2))", "x^2-3*x+2"];
...
s = ["divisors(12)", "(1,2,3,4,6,12)", "divisors(-12)", "(1,2,3,4,6,12)", "divisors(a)", "(1,a)", "divisors(-a)", "(1,a)", "divisors(+3*x+3)", "(1,3,1+x,3+3*x)", "divisors(+3*x-3)", "(1,3,-3+3*x,-1+x)", "divisors(-3*x+3)", "(1,3,1-x,3-3*x)", "divisors(-3*x-3)", "(1,3,1+x,3+3*x)"];
...
s = ["bake=0", "", "factor((x+1)*(x+2)*(x+3),x)", "(1+x)*(2+x)*(3+x)", "factor((x+a)*(x^2+x+1),x)", "(1+x+x^2)*(a+x)", "factor(x*(x+1)*(x+2),x)", "x*(1+x)*(2+x)", "factor((-2*x+3)*(x+4),x)", "-(-3+2*x)*(4+x)", "(-2*x+3)*(x+4)+(-3+2*x)*(4+x)", "0", "factor((x+1)*(-x^2+x+1),x)", "-(-1-x+x^2)*(1+x)", "factor((x+1/2)*(+x+1/3)*(+x+1/4),x)", "1/24*(1+2*x)*(1+3*x)*(1+4*x)", "(x+1/2)*(+x+1/3)*(+x+1/4)-1/24*(1+2*x)*(1+3*x)*(1+4*x)", "0", "factor((x+1/2)*(+x+1/3)*(+x-1/4),x)", "1/24*(-1+4*x)*(1+2*x)*(1+3*x)", "(x+1/2)*(+x+1/3)*(+x-1/4)-1/24*(-1+4*x)*(1+2*x)*(1+3*x)", "0", "factor((x+1/2)*(+x+1/3)*(-x+1/4),x)", "-1/24*(-1+4*x)*(1+2*x)*(1+3*x)", "(x+1/2)*(+x+1/3)*(-x+1/4)+1/24*(-1+4*x)*(1+2*x)*(1+3*x)", "0", "factor((x+1/2)*(+x+1/3)*(-x-1/4),x)", "-1/24*(1+2*x)*(1+3*x)*(1+4*x)", "(x+1/2)*(+x+1/3)*(-x-1/4)+1/24*(1+2*x)*(1+3*x)*(1+4*x)", "0", "factor((x+1/2)*(+x-1/3)*(+x+1/4),x)", "1/24*(-1+3*x)*(1+2*x)*(1+4*x)", "(x+1/2)*(+x-1/3)*(+x+1/4)-1/24*(-1+3*x)*(1+2*x)*(1+4*x)", "0", "factor((x+1/2)*(+x-1/3)*(+x-1/4),x)", "1/24*(-1+3*x)*(-1+4*x)*(1+2*x)", "(x+1/2)*(+x-1/3)*(+x-1/4)-1/24*(-1+3*x)*(-1+4*x)*(1+2*x)", "0", "factor((x+1/2)*(+x-1/3)*(-x+1/4),x)", "-1/24*(-1+3*x)*(-1+4*x)*(1+2*x)", "(x+1/2)*(+x-1/3)*(-x+1/4)+1/24*(-1+3*x)*(-1+4*x)*(1+2*x)", "0", "factor((x+1/2)*(+x-1/3)*(-x-1/4),x)", "-1/24*(-1+3*x)*(1+2*x)*(1+4*x)", "(x+1/2)*(+x-1/3)*(-x-1/4)+1/24*(-1+3*x)*(1+2*x)*(1+4*x)", "0", "factor((x+1/2)*(-x+1/3)*(+x+1/4),x)", "-1/24*(-1+3*x)*(1+2*x)*(1+4*x)", "(x+1/2)*(-x+1/3)*(+x+1/4)+1/24*(-1+3*x)*(1+2*x)*(1+4*x)", "0", "factor((x+1/2)*(-x+1/3)*(+x-1/4),x)", "-1/24*(-1+3*x)*(-1+4*x)*(1+2*x)", "(x+1/2)*(-x+1/3)*(+x-1/4)+1/24*(-1+3*x)*(-1+4*x)*(1+2*x)", "0", "factor((x+1/2)*(-x+1/3)*(-x+1/4),x)", "1/24*(-1+3*x)*(-1+4*x)*(1+2*x)", "(x+1/2)*(-x+1/3)*(-x+1/4)-1/24*(-1+3*x)*(-1+4*x)*(1+2*x)", "0", "factor((x+1/2)*(-x+1/3)*(-x-1/4),x)", "1/24*(-1+3*x)*(1+2*x)*(1+4*x)", "(x+1/2)*(-x+1/3)*(-x-1/4)-1/24*(-1+3*x)*(1+2*x)*(1+4*x)", "0", "factor((x+1/2)*(-x-1/3)*(+x+1/4),x)", "-1/24*(1+2*x)*(1+3*x)*(1+4*x)", "(x+1/2)*(-x-1/3)*(+x+1/4)+1/24*(1+2*x)*(1+3*x)*(1+4*x)", "0", "factor((x+1/2)*(-x-1/3)*(+x-1/4),x)", "-1/24*(-1+4*x)*(1+2*x)*(1+3*x)", "(x+1/2)*(-x-1/3)*(+x-1/4)+1/24*(-1+4*x)*(1+2*x)*(1+3*x)", "0", "factor((x+1/2)*(-x-1/3)*(-x+1/4),x)", "1/24*(-1+4*x)*(1+2*x)*(1+3*x)", "(x+1/2)*(-x-1/3)*(-x+1/4)-1/24*(-1+4*x)*(1+2*x)*(1+3*x)", "0", "factor((x+1/2)*(-x-1/3)*(-x-1/4),x)", "1/24*(1+2*x)*(1+3*x)*(1+4*x)", "(x+1/2)*(-x-1/3)*(-x-1/4)-1/24*(1+2*x)*(1+3*x)*(1+4*x)", "0", "factor((+x+a)*(+x+b)*(+x+c),x)", "(a+x)*(b+x)*(c+x)", "(a+x)*(b+x)*(c+x)-(a+x)*(b+x)*(c+x)", "0", "factor((+x+a)*(+x+b)*(+x-c),x)", "(a+x)*(b+x)*(-c+x)", "(+x+a)*(+x+b)*(+x-c)-(a+x)*(b+x)*(-c+x)", "0", "factor((+x+a)*(+x+b)*(-x+c),x)", "-(a+x)*(b+x)*(-c+x)", "(+x+a)*(+x+b)*(-x+c)+(a+x)*(b+x)*(-c+x)", "0", "factor((+x+a)*(+x+b)*(-x-c),x)", "-(a+x)*(b+x)*(c+x)", "(+x+a)*(+x+b)*(-x-c)+(a+x)*(b+x)*(c+x)", "0", "factor((+a*x+b)*(+c*x+d),x)", "(b+a*x)*(d+c*x)", "(+a*x+b)*(+c*x+d)-(b+a*x)*(d+c*x)", "0", "factor((+a*x+b)*(+c*x-d),x)", "(b+a*x)*(-d+c*x)", "(+a*x+b)*(+c*x-d)-(b+a*x)*(-d+c*x)", "0", "factor((+a*x+b)*(-c*x+d),x)", "-(b+a*x)*(-d+c*x)", "(+a*x+b)*(-c*x+d)+(b+a*x)*(-d+c*x)", "0", "factor((+a*x+b)*(-c*x-d),x)", "-(b+a*x)*(d+c*x)", "(+a*x+b)*(-c*x-d)+(b+a*x)*(d+c*x)", "0", "factor((+a*x-b)*(+c*x+d),x)", "(d+c*x)*(-b+a*x)", "(+a*x-b)*(+c*x+d)-(d+c*x)*(-b+a*x)", "0", "factor((+a*x-b)*(+c*x-d),x)", "(-b+a*x)*(-d+c*x)", "(+a*x-b)*(+c*x-d)-(-b+a*x)*(-d+c*x)", "0", "factor((+a*x-b)*(-c*x+d),x)", "-(-b+a*x)*(-d+c*x)", "(+a*x-b)*(-c*x+d)+(-b+a*x)*(-d+c*x)", "0", "factor((+a*x-b)*(-c*x-d),x)", "-(d+c*x)*(-b+a*x)", "(+a*x-b)*(-c*x-d)+(d+c*x)*(-b+a*x)", "0", "factor((-a*x+b)*(+c*x+d),x)", "-(d+c*x)*(-b+a*x)", "(-a*x+b)*(+c*x+d)+(d+c*x)*(-b+a*x)", "0", "factor((-a*x+b)*(+c*x-d),x)", "-(-b+a*x)*(-d+c*x)", "(-a*x+b)*(+c*x-d)+(-b+a*x)*(-d+c*x)", "0", "factor((-a*x+b)*(-c*x+d),x)", "(-b+a*x)*(-d+c*x)", "(-a*x+b)*(-c*x+d)-(-b+a*x)*(-d+c*x)", "0", "factor((-a*x+b)*(-c*x-d),x)", "(d+c*x)*(-b+a*x)", "(-a*x+b)*(-c*x-d)-(d+c*x)*(-b+a*x)", "0", "factor((-a*x-b)*(+c*x+d),x)", "-(b+a*x)*(d+c*x)", "(-a*x-b)*(+c*x+d)+(b+a*x)*(d+c*x)", "0", "factor((-a*x-b)*(+c*x-d),x)", "-(b+a*x)*(-d+c*x)", "(-a*x-b)*(+c*x-d)+(b+a*x)*(-d+c*x)", "0", "factor((-a*x-b)*(-c*x+d),x)", "(b+a*x)*(-d+c*x)", "(-a*x-b)*(-c*x+d)-(b+a*x)*(-d+c*x)", "0", "factor((-a*x-b)*(-c*x-d),x)", "(b+a*x)*(d+c*x)", "(-a*x-b)*(-c*x-d)-(b+a*x)*(d+c*x)", "0", "factor(x,x)", "x", "factor(x^2,x)", "x^2", "factor(x^3,x)", "x^3", "bake=1", "", "y=(x+1)(x+2)", "", "factor(y,z)", "x^2+3*x+2", "factor(y,y)", "x^2+3*x+2", "y=x^2+exp(x)", "", "factor(y)", "x^2+exp(x)"];
...
s = ["gcd(30,42)", "6", "gcd(42,30)", "6", "gcd(-30,42)", "6", "gcd(42,-30)", "6", "gcd(30,-42)", "6", "gcd(-42,30)", "6", "gcd(-30,-42)", "6", "gcd(-42,-30)", "6", "gcd(x,x)", "x", "gcd(-x,x)", "x", "gcd(x,-x)", "x", "gcd(-x,-x)", "-x", "gcd(x^2,x^3)", "x^2", "gcd(x,y)", "1", "gcd(y,x)", "1", "gcd(x*y,y)", "y", "gcd(x*y,y^2)", "y", "gcd(x^2*y^2,x^3*y^3)", "x^2*y^2", "gcd(x^2,x^3)", "x^2", "gcd(x+y,x+z)", "1", "gcd(x+y,x+y)", "x+y", "gcd(x+y,2*x+2*y)", "x+y", "gcd(-x-y,x+y)", "x+y", "gcd(4*x+4*y,6*x+6*y)", "2*x+2*y", "gcd(4*x+4*y+4,6*x+6*y+6)", "2+2*x+2*y", "gcd(4*x+4*y+4,6*x+6*y+12)", "1", "gcd(27*t^3+y^3+9*t*y^2+27*t^2*y,t+y)", "1", "gcd(2*a^2*x^2+a*x+a*b,a)", "a", "gcd(2*a^2*x^2+a*x+a*b,a^2)", "a", "gcd(2*a^2*x^2+2*a*x+2*a*b,a)", "a", "gcd(2*a^2*x^2+2*a*x+2*a*b,2*a)", "2*a", "gcd(2*a^2*x^2+2*a*x+2*a*b,3*a)", "a", "gcd(2*a^2*x^2+2*a*x+2*a*b,4*a)", "2*a", "gcd(x,x^2)", "x", "gcd(x,x^a)", "1", "gcd(12,18,9)", "3"];
...
s = ["imag(a+i*b)", "b", "imag(1+exp(i*pi/3))", "1/2*3^(1/2)", "imag(i)", "1", "imag((-1)^(1/3))", "1/2*3^(1/2)", "imag(-i)", "-1"];
...
s = ["lcm(4,6)", "12", "lcm(4*x,6*x*y)", "12*x*y", "lcm(2,3,4)", "12"];
...
s = ["mag(a+i*b)", "(a^2+b^2)^(1/2)", "mag(exp(a+i*b))", "exp(a)", "mag(1)", "1", "mag(-1)", "1", "mag(1+exp(i*pi/3))", "3^(1/2)", "mag((a+i*b)/(c+i*d))", "(a^2+b^2)^(1/2)/((c^2+d^2)^(1/2))", "mag(exp(i theta))", "1", "mag(exp(-i theta))", "1", "mag((-1)^theta)", "1", "mag((-1)^(-theta))", "1", "mag(3*(-1)^theta)", "3", "mag(3*(-1)^(-theta))", "3", "mag(-3*(-1)^theta)", "3", "mag(-3*(-1)^(-theta))", "3"];
...
test_multiply = function() {
  ...
  return test(["0*a", "0", "a*0", "0", "1*a", "a", "a*1", "a", "a*a", "a^2", "a^2*a", "a^3", "a*a^2", "a^3", "a^2*a^2", "a^4", "2^a*2^(3-a)", "8", "sqrt(2)/2", "2^(-1/2)", "2/sqrt(2)", "2^(1/2)", "-sqrt(2)/2", "-1/(2^(1/2))", "2^(1/2-a)*2^a/10", "1/(5*2^(1/2))", "i/4", "1/4*i", "1/(4 i)", "-1/4*i", "1.0 pi 1/2", "0.5*pi", "1.0 1/2 pi", "0.5*pi"]);
};
...
s = ["numerator(2/3)", "2", "numerator(x)", "x", "numerator(1/x)", "1", "numerator(a+b)", "a+b", "numerator(1/a+1/b)", "a+b"];
...
yyouter = function() {
  var ad, ae, af, ag, i, j, k, ndim, nelem, ref1, ref2, ref3, ref4;
  i = 0;
  j = 0;
  k = 0;
  ndim = 0;
  ...
  j = i;
  for (i = ae = 0, ref2 = p2.tensor.ndim; 0 <= ref2 ? ae < ref2 : ae > ref2; i = 0 <= ref2 ? ++ae : --ae) {
...
power_sum = function(n) {
  var a, ad, ae, af, i, j, k, ref1, ref2, ref3;
  a = [];
  i = 0;
  j = 0;
  k = 0;
  ...
  push_frame(k * (n + 1));
  p1 = cdr(p1);
  for (i = ad = 0, ref1 = k; 0 <= ref1 ? ad < ref1 : ad > ref1; i = 0 <= ref1 ? ++ad : --ad) {
  ...
  factorial();
  p1 = pop();
  for (i = af = 0, ref3 = k; 0 <= ref3 ? af < ref3 : af > ref3; i = 0 <= ref3 ? ++af : --af) {
    a[i] = 0;
  }
  push(zero);
...
multinomial_sum = function(k, n, a, i, m) {
  var ad, ae, af, j, ref1, ref2, ref3;
  j = 0;
  if (i < k - 1) {
    for (j = ad = 0, ref1 = m; 0 <= ref1 ? ad <= ref1 : ad >= ref1; j = 0 <= ref1 ? ++ad : --ad) {
      a[i] = j;
  ...
  a[i] = m;
  push(p1);
  for (j = ae = 0, ref2 = k; 0 <= ref2 ? ae < ref2 : ae > ref2; j = 0 <= ref2 ? ++ae : --ae) {
    push_integer(a[j]);
    factorial();
    divide();
  }
...
quickfactor = function() {
  ...
  n = tos - h;
  stackIndex = h;
  for (i = ad = 0, ref1 = n; ad < ref1; i = ad += 2) {
...
test_quickfactor = function() {
  var ad, base, expo, h, i, j;
  i = 0;
  logout("testing quickfactor\n");
  for (i = ad = 2; ad < 10001; i = ++ad) {
    if (i % 1000 === 0) {
      alert(i);
    }
    base = i;
...
s = ["real(a+i*b)", "a", "real(1+exp(i*pi/3))", "3/2", "real(i)", "0", "real((-1)^(1/3))", "1/2"];
...
s = ["rect(a+i*b)", "a+i*b", "rect(exp(a+i*b))", "i*exp(a)*sin(b)+exp(a)*cos(b)", "rect(1+exp(i*pi/3))", "3/2+1/2*i*3^(1/2)", "z=(a+b*i)/(c+d*i)", "", "rect(z)-real(z)-i*imag(z)", "0", "z=quote(z)", ""];
...
run_test = function(s) {
  var ad, i, ref1, resultFromRun, t;
  i = 0;
  t = "";
  test_flag = 1;
  run("clear");
  run("e=quote(e)");
  for (i = ad = 0, ref1 = s.length; ad < ref1; i = ad += 2) {
...
Eval_tensor = function() {
  var a, ad, ae, b, i, ndim, nelem, ref1, ref2;
  i = 0;
  ndim = 0;
  nelem = 0;
  if (p1.tensor.nelem !== p1.tensor.elem.length) {
  ...
  p2 = alloc_tensor(nelem);
  p2.tensor.ndim = ndim;
  for (i = ad = 0, ref1 = ndim; 0 <= ref1 ? ad < ref1 : ad > ref1; i = 0 <= ref1 ? ++ad : --ad) {
    p2.tensor.dim[i] = p1.tensor.dim[i];
  }
  a = p1.tensor.elem;
  b = p2.tensor.elem;
...
tensor_plus_tensor = function() {
  var a, ad, ae, af, b, c, i, ndim, nelem, ref1, ref2, ref3;
  i = 0;
  ndim = 0;
  nelem = 0;
  save();
...
d_tensor_tensor = function() {
  var a, ad, ae, af, b, c, i, j, ndim, nelem, ref1, ref2, ref3;
  i = 0;
  j = 0;
  ndim = 0;
  nelem = 0;
  ...
  c = p3.tensor.elem;
  for (i = ae = 0, ref2 = nelem; 0 <= ref2 ? ae < ref2 : ae > ref2; i = 0 <= ref2 ? ++ae : --ae) {
    for (j = af = 0, ref3 = p2.tensor.nelem; 0 <= ref3 ? af < ref3 : af > ref3; j = 0 <= ref3 ? ++af : --af) {
      push(a[i]);
      push(b[j]);
      derivative();
...
d_scalar_tensor = function() {
  ...
  a = p2.tensor.elem;
  b = p3.tensor.elem;
  for (i = ad = 0, ref1 = p2.tensor.nelem; 0 <= ref1 ? ad < ref1 : ad > ref1; i = 0 <= ref1 ? ++ad : --ad) {
    push(p1);
    push(a[i]);
    derivative();
    b[i] = pop();
...
power_tensor = function() {
  var ad, ae, i, k, n, ref1, ref2, results;
  i = 0;
  k = 0;
  n = 0;
  k = p1.tensor.ndim - 1;
  ...
  push(p1);
  results = [];
  for (i = ae = 1, ref2 = n; 1 <= ref2 ? ae < ref2 : ae > ref2; i = 1 <= ref2 ? ++ae : --ae) {
    push(p1);
    inner();
...
promote_tensor = function() {
  var ad, ae, af, ag, ah, i, j, k, ndim, nelem, ref1, ref2, ref3, ref4, ref5;
  i = 0;
  j = 0;
  k = 0;
  nelem = 0;
  ...
  p2 = p1.tensor.elem[0];
  for (i = ad = 1, ref1 = p1.tensor.nelem; 1 <= ref1 ? ad < ref1 : ad > ref1; i = 1 <= ref1 ? ++ad : --ad) {
  ...
  for (j = af = 0, ref3 = p2.tensor.ndim; 0 <= ref3 ? af < ref3 : af > ref3; j = 0 <= ref3 ? ++af : --af) {
...
compatible = function(p, q) {
  ...
  for (i = ad = 0, ref1 = p.tensor.ndim; 0 <= ref1 ? ad < ref1 : ad > ref1; i = 0 <= ref1 ? ++ad : --ad) {
...
transform = function(s) {
  ...
  push(p4);
  decomp();
  for (ad = 0, len1 = s.length; ad < len1; ad++) {
    eachEntry = s[ad];
...
f_equals_a = function(h) {
  var ad, ae, i, j, ref1, ref2, ref3, ref4;
  i = 0;
  j = 0;
  for (i = ad = ref1 = h, ref2 = tos; ref1 <= ref2 ? ad < ref2 : ad > ref2; i = ref1 <= ref2 ? ++ad : --ad) {
    set_binding(symbol(METAA), stack[i]);
    for (j = ae = ref3 = h, ref4 = tos; ref3 <= ref4 ? ae < ref4 : ae > ref4; j = ref3 <= ref4 ? ++ae : --ae) {
